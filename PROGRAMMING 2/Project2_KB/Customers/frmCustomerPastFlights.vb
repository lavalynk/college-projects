Public Class frmCustomerPastFlights
    Private Sub frmCustomerPastFlights_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ' Open the database connection (this is in the module)
            ...
            strSelect = "SELECT SUM(TF.intMilesFlown) AS TotalMilesFlown " &
                        "FROM TFlights TF " &
                        "JOIN TFlightPassengers TFP ON TF.intFlightID = TFP.intFlightID " &
                        "JOIN TPassengers TP ON TP.intPassengerID = TFP.intPassengerID " &
            ...
            ' Build the select statement for past flights, now including the TSeats table
            strSelect = "SELECT TF.intFlightID, TF.strFlightNumber, TF.dtmFlightDate, TF.dtmTimeofDeparture, TF.dtmTimeOfLanding, " &
                        "FA.strAirportCode AS strFromAirportCode, TA.strAirportCode AS strToAirportCode, TF.intMilesFlown, TSE.strSeat " &
                        "FROM TFlights TF " &
                        "JOIN TFlightPassengers TFP ON TF.intFlightID = TFP.intFlightID " &
                        "JOIN TPassengers TP ON TP.intPassengerID = TFP.intPassengerID " &
                        "JOIN TAirports FA ON FA.intAirportID = TF.intFromAirportID " &
                        "JOIN TAirports TA ON TA.intAirportID = TF.intToAirportID " &
                        "JOIN TSeats TSE ON TFP.intSeatID = TSE.intSeatID " &
                        "WHERE TF.dtmFlightDate < CAST(GETDATE() AS DATE) " &
                        "AND TP.intPassengerID = " & intPassenger & " " &
                        "GROUP BY TF.intFlightID, TF.strFlightNumber, TF.dtmFlightDate, TF.dtmTimeofDeparture, TF.dtmTimeOfLanding, " &
                        "FA.strAirportCode, TA.strAirportCode, TF.intMilesFlown, TSE.strSeat"
            ...
            While drSourceTable.Read()
                ...
                lstPastFlights.Items.Add("Seat: " & vbTab & vbTab & drSourceTable("strSeat"))
                lstPastFlights.Items.Add("  ")
                lstPastFlights.Items.Add("=============================")
            End While
